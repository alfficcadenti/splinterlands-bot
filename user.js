...
const getPlayerCards = (username) => (fetch(`https://game-api.splinterlands.io/cards/collection/${username}`,
  ...
  .then(x => x && x.json())
  .then(x => x['cards'] ? x['cards'].filter(x=>x.delegated_to === '' || x.delegated_to === username).map(card => card.card_detail_id) : '')
